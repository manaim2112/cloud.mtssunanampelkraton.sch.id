...
export default function LacakCbt() {
  ...
  const calculateNilai = (iduser: number) => {
    ...
    sortData.forEach((v, k) => {
      if (!soal) return;

      let answer;
      if (typeof soal[k].answer === "string") {
        answer = JSON.parse(soal[k].answer);
      }
      if (typeof soal[k].answer === "object") {
        answer = soal[k].answer;
      }

      const tipe = soal[k].tipe;
      if (tipe === "pilgan" && v[1]) {
        const value = Number(v[1]);
        const index = alpha[value];
        if (answer.includes(index)) {
         poin.push(Number(soal[k].score));
        }
      }
    });
    if(poin.length < 1) return 0;
    return poin.reduce((a, b) => a+b);
  };
    ...
    Swal.fire({
      ...
      if (id && sesiActive && ruangActive && flashUser) {
        ...
        window.location.replace(
          pathPrintKehadiran(
            id,
            sesiActive?.name.trim(),
            ruangActive?.name.trim(),
            proktor,
            e.value
          )
        );
  ...
  const reset = (id: number, name: string) => {
    Swal.fire({
      ...
      inputValidator: (value) => {
        ...
        if (flashUser?.jabatan !== "operator") {
          if (value !== "lab" + flashUser?.walikelas)
            return "Maaf Tidak bisa mereset Ulang";

          if (Number(ruangActive?.name.trim()) !== Number(flashUser?.walikelas))
            return "Maaf Anda Tidak berhak mereset, silahkan reset sesuai ruang masing masing";
        } else {
            ...
            <div className="order-1 lg:order-2">
              <Card className="overflow-hidden" x-chunk="dashboard-05-chunk-4">
                ...
                <CardContent className="p-6 text-sm">
                  ...
                  <div className="mt-4">
                    <Button onClick={handlePdfKehadiran} className="m-2">
                      Cetak Kehadiran
                    </Button>
                    <Button onClick={handlePdfBeritaAcara} className="m-2">
                      Cetak Berita Acara
                    </Button>
                  </div>
                </CardContent>
